...
function pageDisplay($main)
{
   ...
   if (preg_match('/^(pve|pvp)$/i',$simulateType) && $attackerName && $defenderName) {
      ...
      $main->logger('buildSimulation',array('type' => $results['type'], 'defender' => $results['defender']['name'], 
...
{
   $pageInput      = $main->var('pageInput');
   $playerGearList = $main->getPlayerGearList();
   ...
   foreach ($main->obj('constants')->gearTypes() as $gearType => $gearTypeLabel) {
      ...
      $gearEntry = $playerGearList[$gearId];
      $gearStats = json_decode($gearEntry['stats'],true);
      $gearName  = $gearEntry['name'];
      $gearLevel = $gearStats['level'];

      // We will move the level attribute to the top of the array, and don't need it here
      unset($gearStats['level']);

      $equipMap[$gearType] = array(
         'name'  => $gearName,
         'data'  => $gearStats,
         'level' => (int)$gearLevel ?: 0, 
      );
   }
